...
export const deviceDatasourceDiscoveryAuditLogs = async ({
     status,
     url,
     deviceId,
     dataSourceNames = [],
...
export const deviceDatasourceInstanceDiscoveryAuditLog = async ({
     status,
     url,
     deviceId,
     deviceDataSourceId,
...
export const metricUnitDiscoveryAuditLog = async ({
     status,
     url,
     deviceId,
     deviceDataSourceId,
     ...
     const auditLogData = status === 200
          ? {
               timestamp: new Date().toISOString(),
               status: 'Success',
               details: {
                    message: `Metric Unit Discovery Successful with status code ${status}`,
                    URL: url,
                    Device_ID: deviceId,
                    DataSource_ID: deviceDataSourceId,
          ...
          : {
               ...
               status: 'Failure',
               details: {
                    message: `Metric Unit Discovery Failed. Status Code: ${status}. Error: ${errMessage}`,
                    URL: url,
                    Device_ID: deviceId,
                    DataSource_ID: deviceDataSourceId,
                    Instance_ID: instanceId,
     ...
     try {
          await auditLog(auditLogData, 'Fetch Metric Unit');
     } catch (error) {
          loggerInstance.error('Failed to get Metric Unit', error);
     }
};
